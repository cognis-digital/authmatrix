...
[![PyPI](https://img.shields.io/pypi/v/cognis-authmatrix.svg)](https://pypi.org/project/cognis-authmatrix/)
[![CI](https://github.com/cognis-digital/authmatrix/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/authmatrix/actions)
...
AUTHMATRIX exists for one job — test an access-control matrix (role x endpoint) for idor/authz gaps — and does it without a SaaS bill or heavyweight setup.
...
```bash
authmatrix --version
authmatrix scan .                      # scan the current project
authmatrix scan . --format json
authmatrix scan . --fail-on high       # non-zero exit for CI gates
authmatrix mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **AuthMatrix**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`authmatrix` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
