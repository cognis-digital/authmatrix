...
- **rules/analyzers** apply the heuristics shipped in `authmatrix/core.py`.
- **score** ranks by severity.
- **MCP server** (`authmatrix mcp`) exposes `scan` for Cognis.Studio agents.
