"""Native cognis-connect emit for authmatrix — forward findings to any platform.

Maps authmatrix's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    authmatrix ... --format json | authmatrix-emit --to stix
    authmatrix-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('kind', None)
        out['title'] = f"IDOR in {rec.get('role')} for {rec.get('endpoint')}"
        out['severity'] = rec.get('severity', 'low')
        out['type'] = 'id_or'
        out['description'] = f"Role {rec.get('role')} can access endpoint {rec.get('endpoint')} with different permissions."
        out['tags'] = ['id_or', 'authorization']
